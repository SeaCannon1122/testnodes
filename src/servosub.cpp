...
{
public:
  servosub() : Node("servosub") {
    servo_cmd_sub_ = this->create_subscription<geometry_msgs::msg::Twist>("/n10/servo_cmd_vel", 10, std::bind(&servosub::servo_cmd_callback, this, std::placeholders::_1));
    ...
    void servo_cmd_callback(const geometry_msgs::msg::Twist::SharedPtr msg) {
...
private:
  rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr servo_cmd_sub_;
...
int main(int argc, char * argv[])
{
  ...
  auto node = std::make_shared<servosub>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
